...
fn main() {
                ...
                if let Some(path_type) = path_type {
                    ...
                    loop {
                        ...
                        if let Some((nx, ny, r, d)) = m {
                            ...
                            bounds[0] = cmp::min(bounds[0], px as u32);
                            bounds[1] = cmp::min(bounds[1], py as u32);
                            bounds[2] = cmp::max(bounds[2], px as u32);
                            bounds[3] = cmp::max(bounds[3], py as u32);
    ...
    let (lw, lh) = ((bounds[2] - bounds[0]) as i32 / 2, (bounds[3] - bounds[1]) as i32 / 2);
    for p in paths {
        ...
        // Calculate bounds and center of lights and spawns
            ...
            for &(x, y, _, _) in &p.pixels {
                bounds[0] = cmp::min(bounds[0], x);
                bounds[1] = cmp::min(bounds[1], y);
                bounds[2] = cmp::max(bounds[2], x);
                bounds[3] = cmp::max(bounds[3], y);
            }

            let (w, h) = (bounds[2] - bounds[0], bounds[3] - bounds[1]);
            let (x, y) = (bounds[0] + w / 2, bounds[1] + h / 2);
