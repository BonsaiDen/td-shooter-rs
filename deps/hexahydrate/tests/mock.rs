...
use hexahydrate::{Entity, ConnectionSlot};
...
impl Entity<TestUserData> for TestEntity {
    ...
    fn filter(&self, _: &ConnectionSlot<TestUserData>) -> bool {
    ...
    fn part_bytes(&mut self, connection_slot: Option<&ConnectionSlot<TestUserData>>) -> Option<Vec<u8>> {
    ...
    fn merge_bytes(&mut self, connection_slot: Option<&ConnectionSlot<TestUserData>>, bytes: &[u8]) {
        assert_eq!(connection_slot.is_some(), self.server_entity);
    ...
    fn to_bytes(&self, connection_slot: &ConnectionSlot<TestUserData>) -> Vec<u8> {
