...
fn test_server_create() {
    Server::<ServerEntity, TestUserData>::new(5);
}

#[test]
fn test_server_with_entities() {

    let mut server = Server::<ServerEntity, TestUserData>::new(3);
...
fn test_server_exhaustive_create_entity() {
    ...
    assert_eq!(stats.lock().unwrap().new, 256);

    assert_eq!(server.entity_create_with(|| Box::new(TestEntity::new(true, stats.clone()))), Err(ServerError::AllEntitySlotsInUse));
    assert_eq!(stats.lock().unwrap().new, 256);
    ...
    let mut count = 0;
    server.update_with(|_, entity| { entity.server_update(&mut count); });
    assert_eq!(count, 255);
    ...
    assert_eq!(stats.lock().unwrap().new, 257);
...
fn test_server_exhaustive_connection_add_remove() {
    ...
    let mut server = Server::<ServerEntity, TestUserData>::new(5);

    let mut connection_slot = None;
    for i in 0..256 {
        let slot = server.connection_add(TestUserData::new(i as u8));
        assert!(slot.is_ok());
        connection_slot = Some(slot);
    }

    assert_eq!(server.connection_add(TestUserData::new(0)), Err(ServerError::AllConnectionSlotsInUse));
    assert_eq!(server.connection_remove(connection_slot.unwrap().unwrap()), Ok(TestUserData::new(255)));
    assert!(server.connection_add(TestUserData::new(0)).is_ok());
...
fn test_server_exhaustive_connection_slot_unique() {
    ...
    let mut server_one = Server::<ServerEntity, TestUserData>::new(5);
    let mut server_two = Server::<ServerEntity, TestUserData>::new(5);
    let connection_one = server_one.connection_add(TestUserData::new(0)).unwrap();
    let connection_two = server_two.connection_add(TestUserData::new(0)).unwrap();

    assert_eq!(connection_one, connection_one);
    assert_ne!(connection_one, connection_two);
    assert_eq!(server_two.connection_remove(connection_one), Err(ServerError::ConnectionDoesNotExist));
...
fn test_server_exhaustive_entity_slot_unique() {
    ...
    assert_eq!(server_two.entity_destroy(entity_one), Err(ServerError::EntityDoesNotExist));
...
fn test_server_connection_send_packet_split() {
    ...
    let connection_one = server.connection_add(TestUserData::new(32)).unwrap();
...
fn test_server_disconnected_entity() {
    ...
    assert_eq!(stats.lock().unwrap().drop_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().drop_calls, 1);
    ...
    // Entity should be dropped right away
    assert_eq!(stats.lock().unwrap().drop_calls, 2);
...
fn test_server_connection_add_existing_entity() {
    ...
    let connection_one = server.connection_add(TestUserData::new(128)).unwrap();
    let connection_two = server.connection_add(TestUserData::new(255)).unwrap();
...
fn test_server_connection_remove_existing_alive_entity() {
    ...
    let connection_one = server.connection_add(TestUserData::new(0)).unwrap();
    let connection_two = server.connection_add(TestUserData::new(0)).unwrap();
...
fn test_server_entity_flow() {
    ...
    let connection_one = server.connection_add(TestUserData::new(255)).unwrap();
    let entity = server.entity_create_with(|| Box::new(TestEntity::new(true, stats.clone()))).unwrap();
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().part_calls, 1);
    assert_eq!(stats.lock().unwrap().merge_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().part_calls, 2);
    assert_eq!(stats.lock().unwrap().merge_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().part_calls, 3);
    assert_eq!(stats.lock().unwrap().merge_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().merge_calls, 1);
    ...
    assert_eq!(stats.lock().unwrap().merge_calls, 2);
    assert_eq!(stats.lock().unwrap().part_calls, 5);

    // Destroy Entity
    server.entity_destroy(entity).unwrap();
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 1);
    assert_eq!(stats.lock().unwrap().drop_calls, 1);
...
fn test_server_multi_entity_flow() {
    ...
    let connection_one = server.connection_add(TestUserData::new(16)).unwrap();
    ...
    assert_eq!(stats.lock().unwrap().merge_calls, 4);
    assert_eq!(stats.lock().unwrap().part_calls, 12);

    // None -> SendUpdateToClient
    stats.lock().unwrap().part_bytes_value = Some(vec![255, 192, 96]);
    assert_server_send!(server, connection_one, vec![], vec![3, 0, 3, 255, 192, 96]).expect("Server send SendUpdateToClient packet with data bytes.");
    assert_eq!(stats.lock().unwrap().merge_calls, 4);
    assert_eq!(stats.lock().unwrap().part_calls, 14);
...
fn test_server_entity_forget() {
    ...
    let connection_one = server.connection_add(TestUserData::new(96)).unwrap();
...
fn test_server_connection_add_remove() {
    let mut server = Server::<ServerEntity, TestUserData>::new(5);
    let connection_one = server.connection_add(TestUserData::new(0));
...
fn test_server_ignore_invalid_packets() {
    ...
    let mut server = Server::<ServerEntity, TestUserData>::new(5);
    let connection_one = server.connection_add(TestUserData::new(0)).expect("Failed to add client connection to server");;
...
fn create_client(send_timeout: usize) -> (Client<ClientEntity, TestUserData, ClientRegistry>, Rc<Mutex<TestStat>>) {
    let stats: Rc<Mutex<TestStat>> = Rc::new(Mutex::new(TestStat::default()));
    let client = Client::<ClientEntity, TestUserData, ClientRegistry>::new(ClientRegistry {
        stats: stats.clone()
...
fn test_client_reset() {
    ...
    let mut count = 0;
    client.update_with(|_, entity| { entity.client_update(&mut count); });
    assert_eq!(count, 3);

    assert_eq!(stats.lock().unwrap().drop_calls, 0);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
    client.reset();
    assert_eq!(stats.lock().unwrap().drop_calls, 3);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
...
fn test_client_ignore_invalid_packets() {
    ...
    assert_eq!(stats.lock().unwrap().merge_calls, 0);
    assert_eq!(stats.lock().unwrap().drop_calls, 0);
    assert_eq!(stats.lock().unwrap().registry_calls, 0);
...
fn test_client_entity_flow() {
    ...
    assert_client_send!(client, vec![0, 0, 3, 1, 255, 128, 255], vec![1, 0]).expect("Client accepts SendCreateToClient packet.");
    assert_eq!(stats.lock().unwrap().registry_calls, 1);
    assert_eq!(stats.lock().unwrap().created_calls, 0);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);

    let mut count = 0;
    client.update_with(|_, entity| {
    ...
    assert_eq!(stats.lock().unwrap().part_calls, 1);
    assert_eq!(stats.lock().unwrap().merge_calls, 1);
    assert_eq!(stats.lock().unwrap().merge_bytes_value, vec![]);
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().part_calls, 2);
    assert_eq!(stats.lock().unwrap().merge_calls, 2);
    assert_eq!(stats.lock().unwrap().merge_bytes_value, vec![255, 192, 96]);
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().drop_calls, 1); // Entity is dropped directly
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 1);
    ...
    assert_eq!(stats.lock().unwrap().registry_calls, 2);
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 1);
...
fn test_client_entity_create_replace() {
    ...
    assert_eq!(stats.lock().unwrap().registry_calls, 3);
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);
    ...
    assert_eq!(stats.lock().unwrap().registry_calls, 4);
    assert_eq!(stats.lock().unwrap().created_calls, 1);
    assert_eq!(stats.lock().unwrap().destroyed_calls, 0);

    assert_client_send!(client, vec![1, 0], vec![2, 0]).expect("Client accepts ConfirmClientCreate packet.");
    assert_eq!(stats.lock().unwrap().created_calls, 2);
...
fn test_client_multi_entity_flow() {
    ...
    assert_eq!(stats.lock().unwrap().drop_calls, 2); // Entity is dropped directly
    ...
    assert_eq!(stats.lock().unwrap().drop_calls, 3); // Entity is dropped directly
